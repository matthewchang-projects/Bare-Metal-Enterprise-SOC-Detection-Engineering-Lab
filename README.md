...
**A security control that appears enabled in its management interface doesn’t necessarily mean it’s actively working in the underlying engine.** OPNsense’s GUI displayed thousands of Suricata rules as enabled, interfaces were correctly configured, and logging was set up, yet the alerts tab remained completely empty during test traffic. The root cause was that several rule categories, including the NMAP scan detection rules, were still commented out in the underlying rule files even though they were shown as enabled in the GUI. This discrepancy occurred because the enabled state in the interface and the actual file state were not reliably synchronized. To resolve this, the solution was to force a known bad signature against the sensor to obtain a definitive pass or fail, rather than continuing to recheck settings that had already been confirmed correct. The broader lesson here is that a control’s displayed configuration state must be validated against its actual behavior, not assumed from a checkbox.
...
**Network segmentation, as depicted in an architecture diagram, is not valid until it is verified against the actual behavior of the firewall.** During the build process, several discrepancies were discovered between the intended topology and the actual rules permitted by OPNsense. Notably, a DMZ initially had the potential to reach segments it shouldn’t have, and workstations retained internet access despite being supposedly isolated. Neither of these gaps was apparent when reviewing the bridge diagram or the firewall ruleset individually. However, they were only identified through direct testing of segment reachability. Segmentation is a claim about enforced rules, not VM placement, and it must be validated hop by hop rather than assumed from the design document.
...
**Cloning a virtual machine template while it’s still domain-joined results in identity conflicts instead of clean copies.** Before generalizing, WS Template was cloned into all six workstations, so each clone inherited the same computer identity and a stale domain trust relationship. Repeated attempts to fix the resulting authentication failures led to one workstation’s domain membership breaking entirely. The eventual repair required recreating its AD computer account and reestablishing the secure channel. The correct process involves building the template, installing all necessary software, running sysprep with the generalize and oobe flags before converting it to a template, and joining each clone to the domain fresh after cloning.
