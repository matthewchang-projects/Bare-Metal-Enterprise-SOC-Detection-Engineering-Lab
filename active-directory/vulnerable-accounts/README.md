...
New-ADUser `
    ...
    -AccountPassword (ConvertTo-SecureString "Backup456" -AsPlainText -Force) `
    -Enabled $true `
    -PasswordNeverExpires $true `
    -Path "OU=Servers,OU=Corp,DC=corp,DC=local"
...
The domain password policy is intentionally permissive. `PasswordNeverExpires` is set on all accounts and the minimum password length is not enforced to enterprise standards. This allows the weak service account passwords (Service123, Backup456) to exist without policy rejection.
