# Honeypot and DMZ - HONEY01 and ZEEK-01

**Bridge:** vmbr3 (192.168.30.0/24)
**OPNsense interface:** OPT3

The DMZ houses two VMs with distinct but complementary roles. HONEY01 presents false attack surfaces and logs every interaction. ZEEK-01 passively analyzes network traffic and produces structured protocol logs. Both forward logs to Splunk. Neither can initiate connections to any other segment - OPT3 firewall rules permit only outbound log traffic to SIEM-01.
...
Zeek passively monitors traffic on the vmbr3 interface and produces structured logs for every connection, DNS query, HTTP request, and TLS session it sees. Unlike Suricata (signature-based), Zeek produces protocol-level behavioral logs with no inherent alerting. This data feeds into correlation queries in Splunk that look for anomalous protocol behavior that signature rules might not catch.
...
The conn.log shows DNS queries (UDP port 53) from workstations on 192.168.20.x to DC01 at 192.168.10.10. This confirms Zeek is capturing cross-segment traffic that transits through OPNsense.
